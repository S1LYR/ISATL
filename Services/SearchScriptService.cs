    ...
    public class SearchScriptService
    {
        ...
        public async Task ProcessDataAsync(LidarData data)
        {
            try
            {
                ...
                var shpPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, data.ShpFilePath);
                ...
                var startInfo = new ProcessStartInfo
                {
                    ...
                    Arguments = $"{Path.GetFullPath(_scriptPath)} {Path.GetFullPath(lasPath)} {Path.GetFullPath(shpPath)} {Path.GetFullPath(outputPath)}",
